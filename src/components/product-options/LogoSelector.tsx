...
const LogoSelector: React.FC<Props> = ({ 
  ...
  const logoImage = logoMaterial === 'steel' ? currentLogoData?.imgSteel : currentLogoData?.imgBrass;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Logo Modal */}
      {currentLogoData && logoMaterial !== 'none' && logoImage && (
        <SimpleModal
          open={isLogoModalOpen}
          onClose={closeLogoModal}
          title={`Лого ${currentLogoData.name} (${materialText})`}
        >
          <div className="aspect-video w-full rounded-xl overflow-hidden shadow mb-6">
            <img
              src={logoImage}
              alt={`Лого ${currentLogoData.name} (${materialText})`}
